...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var loanTypeField = document.getElementById("loan_type");
  ...
  if (productField != null) {
    productField.addEventListener("change", function () {
      ...
      // loanTypeField.addEventListener("change", function (e) {
      //   let value = e.target.value;
      //   if (value.toLowerCase() == 'top up loan') {
      //     console.log(fields);
...
// Handle form interactions: input formatting and submission

document.addEventListener("DOMContentLoaded", () => {
  const inputs = document.querySelectorAll('.formatted-input');
  const submitButton = document.querySelector('.validate, #submitLoanApplicationForm');
  ...
  inputs.forEach(input => {
    input.addEventListener('input', (e) => {
      ...
      const formattedValue = parseFloat(rawValue || 0).toLocaleString('en-US');
      e.target.value = formattedValue; // Set the formatted value in the input
    });
  });
  ...
  if (submitButton != null) {
    submitButton.addEventListener("click", function (event) {
      ...
      // Remove commas from all formatted inputs before submission
      inputs.forEach(input => {
...
// Show loading overlay on button click
// document.addEventListener('DOMContentLoaded', () => {
//   const buttons = document.querySelectorAll('.show-loading');

//   if (buttons.length > 0) { // Check if elements with the class 'show-loading' exist
//       buttons.forEach(button => {
//           button.addEventListener('click', () => {
//               const loadingOverlay = document.getElementById('loading-overlay');
//               if (loadingOverlay) { // Ensure the loading overlay element exists
//                   loadingOverlay.style.display = 'flex';
...
// handle exceptional_insurance
// document.addEventListener('DOMContentLoaded', function () {
//   const insuranceRadio = document.getElementById('exceptional_insurance');
...
//   const otherRadios = document.querySelectorAll('input[name="request_basis"]:not(#exceptional_insurance)');

//   // Show insurance fields if "Insurance" is selected
...
//   if (otherRadios != null) {
//     // Hide insurance fields if another option is selected
...
document.addEventListener('DOMContentLoaded', function () {
  const insuranceRadio = document.getElementById('exceptional_insurance');
  const amountRadio = document.getElementById('exceptional_amount');
  const durationRadio = document.getElementById('exceptional_duration');
  const processingRadio = document.getElementById('exceptional_processing_fees');
  ...
  if (processingRadio != null){
    processingRadio.addEventListener('change', function (){
      if (processingRadio.checked){
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Get all input fields with class 'product-form'
  var inputs = document.querySelectorAll('.add-commas-remove-commas');

  inputs.forEach(function(input) {
      input.addEventListener('input', function() {
          var value = input.value;
          value = removeCommas(value); // Remove existing commas
          if (!isNaN(value) && value !== '') {
              input.value = addCommas(value); // Add commas
          }
      });
  });

  // Add event listener to the submit button
  const saveButton = document.getElementById('save_product');
  if (saveButton != null) {
    saveButton.addEventListener('click', function () {
      inputs.forEach(function (input) {
        input.value = removeCommas(input.value); // Remove commas before submission
      });
    });
  }
});        
...
// Generic function to add commas.
document.addEventListener("DOMContentLoaded", () => {
  const inputs = document.querySelectorAll('.formatted-input-numeric');
  ...
  // Add event listener to the submit button for this class
  const saveButton = document.getElementById('save-formatted-input');
  if (saveButton != null) {
    saveButton.addEventListener('click', function () {
      inputs.forEach(function (input) {
        input.value = removeCommas(input.value); // Remove commas before submission
